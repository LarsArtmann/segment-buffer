...
| f.2  | Property test: segment_count matches disk             | DONE: `segment_count_matches_disk_across_flush_delete_ops` in `src/property_tests.rs`                                                                 | `01-14` T4       | unreleased |
| f.3  | Loom test: segment_count consistency                   | DONE: `segment_count_self_heals_after_concurrent_flush_and_delete` in `tests/loom.rs`                                                                 | `01-14` T5       | unreleased |
| f.4  | Document segment_count underflow contract              | DONE: field rustdoc documents both wrap scenarios + self-heal via `sync_disk_bytes`                                                                   | `01-14` T1       | unreleased |
| f.6  | Check CI green                                         | DONE: CI is green on master as of 2026-08-04                                                                                                           | —                | —          |
| f.7  | segment_count assertion in append_all test             | DONE: `append_all_auto_flush_increments_segment_count`                                                                                                | `01-14` T2       | unreleased |
...
**Still open:** f.5 (run verify-gate.sh — no session has done this), f.8–10 (release decision, backpressure example — user/product decisions). g.1–3 (type decision `u64` vs `usize`, release timing, standalone accessor) are user decisions. f.13–50 (broader backlog — most items are in ROADMAP or TODO_LIST).
