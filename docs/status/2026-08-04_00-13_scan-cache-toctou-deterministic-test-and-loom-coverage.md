...
| f.2  | Update AGENTS.md loom section — 9→11 tests, add scan-cache coverage                            | DONE: AGENTS.md now says "12 tests", describes scan-cache populate + segment_count coverage | docs-health pass | unreleased |
...
| f.3 | Investigate pre-encoded MockStore for loom runtime | RESOLVED: not tractable without compromising fidelity — documented in loom module doc | `01-14` T9 | unreleased |
| f.4 | Loom test for scan_segments + recover interleaving | RESOLVED: impossible by construction — `recover` is open-time-only, documented on the method | `01-14` T8 | unreleased |
| f.5 | mtime_supported == false scan-cache gap | RESOLVED: FORMALLY ACCEPTED — single-process invariant forbids external mutation; in TODO_LIST resolved decisions | `01-12` session | unreleased |
| f.10 | Clean up the read_from_concurrent_delete_acked loom test sentinel | DONE: named `SENTINEL_ID` const, filtered from assertion | `01-14` T3 | unreleased |
| f.19 | Update docs/DOMAIN_LANGUAGE.md consistency-model to reference Barrier test + loom scan-cache tests | DONE: consistency-model now cites the Barrier test and scan-cache loom tests as proof artifacts | docs-health pass | unreleased |

**Still open:** f.6 (run verify-gate.sh end-to-end — no session has done this), f.7 (push commits — user decision), f.8 (empty-message commits — auto-commit daemon behavior, unresolved), f.9 (cargo clippy --features fuzz — never run by any session), f.11–16 (remaining test quality improvements — invariant arguments as doc comments, Barrier test in release/100×, concurrent property tests), f.18 (CHANGELOG updates — covered for main deliverables), f.20 (verify-gate.sh hardcoded loom count — does not hardcode), f.21–33 (broader backlog — streaming cipher, envelope v2, etc. are in ROADMAP).
