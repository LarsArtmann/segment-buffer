...
# ownership transfers (the pnpm-style compromised-maintainer vector). The
...
- **Code duplication gate (`jscpd`)**: CI job `duplication` scans `src/*.rs` for copy-paste blocks via pnpm-installed `jscpd@3` with a 2% threshold (config: `.jscpd.json`). Locally this is `scripts/check-duplication.sh` (verify-gate gate #19, skipped gracefully if jscpd/jq are absent). Baseline is ~1% — the two intentional clones are the segment encode signatures in `src/segment.rs` and the AEAD output assembly in `src/cipher.rs`. **Gotcha:** jscpd v3's `--exitCode` flag is broken (always exits 0) — the script must keep its jq-based threshold check. Also, CLI flags (`--format rust --min-lines 5 --min-tokens 60`) yield 0 clones while the equivalent `.jscpd.json` config yields 2; the threshold is calibrated against config-file behavior, so run jscpd via the script/config, not raw CLI flags.
- **Nightly benchmarks** (`.github/workflows/bench-nightly.yml`): runs `cargo bench --features encryption` at 04:00 UTC daily (plus `workflow_dispatch`) on ubuntu-latest. Criterion baseline is cached keyed by `run_id` with a `restore-keys` fallback, so each night compares against the previous night; `target/criterion/` is uploaded as an artifact. It does NOT install zstd and does NOT post notifications on regression — the comparison is visible only in the Actions log, so act on performance dips by checking that log.
...
2. **Run the full local gate.** `scripts/verify-gate.sh` — all 19 gates must pass.
3. **Bump `Cargo.toml` version.** Edit `version = "X.Y.Z"`. If a new release touches the `html_root_url`, update the version segment in `src/lib.rs` `#![doc(html_root_url = "...")]`. Then run `cargo check` to sync `Cargo.lock` with the new version, and `cargo publish --dry-run --features encryption` to catch any packaging error before tagging. The `cargo-lock-version` verify-gate asserts the lock file is in sync.
...
4. **Run the verification gate before declaring work done.** The canonical gate is `scripts/verify-gate.sh` (all 19 gates by default: `fmt`, `clippy(default)`, `clippy(encryption)`, `clippy(fuzz)`, `test(default)`, `test(encryption)`, `doc`, `html_root_url`, `cargo-lock`, `cargo-lock-version`, `msrv-consistency`, `cargo-deny`, `cargo-audit`, `loom`, `lychee`, `changelog-links`, `actionlint`, `jscpd`, and `nix flake check`). Use `--list` to print all gate names or `--only=X,Y,Z` to run a subset. The script uses `set -euo pipefail` and the `run()` helper captures the real exit status with `|| rc=$?` so a failing gate does not silently exit 0. Any claim that "tests pass" or "the build is green" must rest on a literal run of this gate (or the equivalent subset if a tool is unavailable) in the current session, with non-zero exit codes captured. The four basic commands (`cargo fmt`, `cargo clippy`, `cargo test`, `cargo doc`) are the minimal subset that can be run by hand when iterating quickly; the full gate is the source of truth for "done" claims.
